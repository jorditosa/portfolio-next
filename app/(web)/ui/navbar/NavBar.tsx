    ...
    <>

      {
        ...
        openedMenu ? (
          ...
          <nav className={`${ openedMenu ? 'z-40' : 'z-[-1]'} absolute left-0  mb-8 h-screen w-full transition-all duration-[1s] ease-in-out`}>
            <ul className="w-full h-screen flex font-semibold text-xs lg:text-xl overflow-hidden">
              <li className={`${ openedMenu ? 'top-[0]' : '-top-[100%]'} relative py-2 flex-1 cursor-pointer gap-8 bg-gradient-to-b from-blue-500 to-white before:absolute before:bg-blue-900 before:w-full before:h-full before:-top-[100%] before:duration-300 before:ease-in transition-all before:hover:top-0 before:z-0 hover:text-white duration-[0.2s] ease-linear`}>
                <Link 
              ...
              <li 
              onClick={() => setOpenedMenu(false)}
              className={`${ openedMenu ? 'top-[0]' : '-top-[100%]'} relative h-full w-full py-2 flex-1 cursor-pointer bg-gradient-to-b from-blue-500 to-white duration-[1s] ease-linear flex flex-col items-center justify-center gap-8`}>
